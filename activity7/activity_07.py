...
def store_chunks(client, collection_name: str, chunks: list[str], strategy: str) -> None:
    ...
    for index, chunk in enumerate(chunks):
        points.append(
            PointStruct(
                id=abs(hash(f"{strategy}-{index}")) % (10 ** 15),
    ...
    print(f"  ✔ Stored {len(points)} chunks using strategy: [{strategy.upper()}]")
...
def main():
    print("=" * 100)
    print("  Activity 7: Qdrant Memory & Chunking Strategy")
    print("=" * 100)
    ...
    print(f"  ✔ Created collection: {COLLECTION_NAME} (dims={VECTOR_SIZE}, metric=COSINE)\n")
    ...
    print(f"\nQuery: \"{query_text}\"")
    print(f"Query vector: {[round(v, 4) for v in query_vector]}")
    ...
    if match:
        ...
        print(f"\n  Best Match Strategy : [ {payload.get('strategy').upper()} ]")
